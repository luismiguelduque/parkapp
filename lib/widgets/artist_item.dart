    ...
    return GestureDetector(
      onTap: () {
        Navigator.of(context)
            .pushNamed("artist-detail", arguments: widget.artist.id);
      },
      child: Container(
        ...
        child: Column(
          ...
          children: [
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                              "${formaterDateTime(widget.artist.requestDate)} hs",
                              style: TextStyle(
                                  color: AppTheme.getTheme().disabledColor,
                                  fontWeight: FontWeight.bold),
                            ),
            ...
            if (_preferences.userTypeId == 3) _adminOptions(),
            SizedBox(
              height: 5,
            ),
          ],
              ...
              if (widget.artist.status == "4")
                CustomGeneralButton(
                  onPressed: () async {
                    ...
                    final ConversationModel conversation = chatProvider
                        .adminAllConversations
                        .firstWhere((element) =>
                            element.user.id == widget.artist.userId);
                    final String _messageText =
                        "¡Gracias por registrarte como artista! Para que el perfil de artista sea activado en tu celular es necesario cerrar sesión desde 'Perfil' y volver a iniciarla poniendo nuevamente usuario y contraseña. ¡Gracias!";
                    final artistsProvider =
                        Provider.of<ArtistsProvider>(context, listen: false);
                    final resp =
                        await artistsProvider.activateArtist(widget.artist.id);
                    setState(() => isSaving = false);
                    if (resp['success']) {
                      showSuccessMessage(context, resp["message"]);
                      await Future.wait([
                        artistsProvider.getArtists(
                            limit: _limit, offset: _offset, search: null),
                        artistsProvider.getArtistsRequests(
                            limit: _limit, offset: _offset, search: null),
                        artistsProvider.getArtistsSuspensions(
                            limit: _limit, offset: _offset, search: null),
                        chatProvider.updateLastMessage(
                            conversation.id,
                            "¡Gracias por registrarte como artista! Para que el perfil de artista sea activado...",
                            _preferences.userId,
              ...
              if (widget.artist.status == "1")
                CustomGeneralButton(
                  onPressed: () async {
                    setState(() => isSaving = true);
                    final resp =
                        await artistsProvider.suspendArtist(widget.artist.id);
                    setState(() => isSaving = false);
                    if (resp['success']) {
                      showSuccessMessage(context, resp["message"]);
                      await Future.wait([
                ...
                Row(
                  children: [
                    CustomGeneralButton(
                      onPressed: () async {
                        setState(() => isSaving = true);
                        final resp = await artistsProvider
                            .reactivateArtist(widget.artist.id);
                        setState(() => isSaving = false);
                        if (resp['success']) {
                          showSuccessMessage(context, resp["message"]);
                          await Future.wait([
                  ...
                  child: Row(
                    ...
                    children: [
                      CustomGeneralButton(
                        onPressed: () async {
                          setState(() => isSaving = true);
                          final artistsProvider = Provider.of<ArtistsProvider>(
                              context,
                              listen: false);
                          final resp = await artistsProvider
                              .rejectArtist(widget.artist.id);
                          setState(() => isSaving = false);
                          if (resp['success']) {
                            showSuccessMessage(context, resp["message"]);
                            await Future.wait([
                      ...
                      CustomGeneralButton(
                        onPressed: () async {
                          ...
                          final artistsProvider = Provider.of<ArtistsProvider>(
                              ...
                              listen: false);
                          final resp = await artistsProvider
                              .activateArtist(widget.artist.id);
                          setState(() => isSaving = false);
                          if (resp['success']) {
                            showSuccessMessage(context, resp["message"]);
                            await Future.wait([
                    ...
                    Row(
                      ...
                      children: [
                        CustomGeneralButton(
                          ...
                          onPressed: () async {
                            setState(() => isSaving = true);
                            final resp = await Provider.of<ArtistsProvider>(
                                    context,
                                    listen: false)
                            ...
                            if (resp['success']) {
                              showSuccessMessage(context, resp["message"]);
                              await Future.wait([
                                Provider.of<ArtistsProvider>(context,
                                        listen: false)
                                    .getArtists(
                                        limit: _limit,
                                        offset: _offset,
