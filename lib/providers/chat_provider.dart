...
class ChatProvider extends ChangeNotifier {
  ...
  Future<void> getAdminAllConversation() async {
    try {
      final Uri uri = developmentMode
          ? Uri.https(
              apiUrl, "api/conversations", {"offset": "0", "limit": "1000"})
          : Uri.http(
              apiUrl, "api/conversations", {"offset": "0", "limit": "1000"});
  ...
  Future<void> getAdminConversation(
      String search, int offset, int limit, int userType) async {
    try {
      final Uri uri = developmentMode
          ? Uri.https(apiUrl, "api/conversations", {
              "search": "$search",
              "offset": "$offset",
              "limit": "$limit",
              "user_type": "$userType"
  ...
  Future<void> getUserConversation() async {
    final Uri uri = developmentMode
        ? Uri.https(apiUrl, "api/conversations/user-session", {})
        : Uri.http(apiUrl, "api/conversations/user-session", {});
    try {
      final response = await http.get(uri, headers: {
        'Content-type': 'application/json',
        'Accept': 'application/json',
      ...
      if (conversations.length > 0) {
        _userConversation =
            ConversationModel.fromJson(extractedData['data'][0]);
      } else {
  ...
  Future<Map<String, dynamic>> updateLastMessage(
      int conversationId, String message, int sender, int receiver) async {
    Map<String, dynamic> respJson = {};
    final bool isAdmin = _preferences.userTypeId == 3;
    final Uri uri = developmentMode
        ? Uri.https(apiUrl, "api/conversations/last-message", {})
        : Uri.http(apiUrl, "api/conversations/last-message", {});
    try {
      final response = await http.post(
        uri,
        headers: {
  ...
  Future<Map<String, dynamic>> setRead(int conversationId) async {
    Map<String, dynamic> respJson = {};
    final Uri uri = developmentMode
        ? Uri.https(apiUrl, "api/conversations/read-messages",
            {"id": "$conversationId"})
